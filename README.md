...
В проекте реализованы unit-тесты для основных компонентов:  
- Service-слой
- Controller-слой
...
- При покрытии ниже порога сборка падает

Общий процент покрытия составляет 69%

Генерация отчёта:  
`mvn verify`  
Отчет можно увидеть: `target/site/jacoco/index.html`.  

## 10. Линтер

Для проверки качества кода используется Checkstyle. Конфигурация находится в:  `checkstyle.xml`.  
Запуск:  
`mvn checkstyle:check` 

Проверяются:  
- Длина строки (не больше 120 символов)  
- Именование локальных переменных, методов  
- Параметры методов  
- Поля классов  
- Отступы, пробелы, пустые строки между методами  
- Лишние импорты  
- Пустые блоки
...
### 2) Lint  
Проверка еода с помощью Checkstyle.
### 3) Test + Coverage  
Запуск тестов и проверка покрытия.
### 4) Docker Build  
Сборка Docker-образа.
### 5) Docker Push  
Выполняется только при push в main. Образ публикуется в Docker Hub.  

Pipeline падает при ошибке сборки, ошибках линтера, падении тестов, coverage ниже 50%.  
Используются secrets для доступа к Docker Hub.  
Docker-образ тегируется хэшем коммита: `todolist:<commit_sha>`.